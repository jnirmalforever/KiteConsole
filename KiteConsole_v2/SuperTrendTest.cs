...
namespace KiteConsole_v2
{
    ...
    {
        public async Task<Position> StartLooking(Settings positionSettings, Kite kite, List<Attributes> Attributes = null)
        {
            ...
            if ((positionSettings.Interval == Constants.INTERVAL_30MINUTE || positionSettings.Interval == Constants.INTERVAL_15MINUTE) && Attributes.Count == 0)
            {
            ...
            else if (string.IsNullOrEmpty(positionSettings.ParentToken) && Attributes.Count == 0)
            ...
            if (Attributes.Count > 0)
            {
                ...
                //List<Attributes> Attributes = historical.OrderBy(x => x.TimeStamp).Select(x => new Attributes
                //{
                //    Close = x.Close,
                ...
                foreach (Attributes att in Attributes)
                {
                    ...
                    {
                        att.ATR = Decimal.Divide(Attributes.OrderBy(x => x.TimeStamp).Skip(1).Take(Constants.SuperTrend).Sum(x => x.TrueRange), Constants.SuperTrend);
                    }
                }
                ...
                //List<Attributes> fAttributes = Attributes.OrderBy(x => x.TimeStamp).Skip(Constants.SuperTrend).Select(x => new Attributes
                //{
                //    Close = x.Close,
                //    TimeStamp = x.TimeStamp,
                //    High = x.High,
                ...
                List<Attributes> fAttributes = Attributes.OrderBy(x => x.TimeStamp).Skip(Constants.SuperTrend).ToList();
                ...
                //{
                //    int i = fAttributes.IndexOf(att);
                //    att.ATR = Decimal.Divide(Attributes.OrderBy(x => x.TimeStamp).Skip(i + 1).Take(Constants.SuperTrend).Sum(x => x.TrueRange), Constants.SuperTrend);
                //}
                ...
                //    if (i > 0)
                //    {
                //        att.ATR = ((fAttributes[i - 1].ATR * Constants.SuperTrend - 1) + att.TrueRange) / Constants.SuperTrend;
                //    }
                //}
                ...
                foreach (Attributes att in fAttributes)
                {
                    ...
                    att.basicUpperBand = Decimal.Divide((att.High + att.Low), 2) + Decimal.Multiply(Convert.ToDecimal(positionSettings.STBasicSetting), att.ATR);
                    att.basicLowerBand = Decimal.Divide((att.High + att.Low), 2) - Decimal.Multiply(Convert.ToDecimal(positionSettings.STBasicSetting), att.ATR);
                }
